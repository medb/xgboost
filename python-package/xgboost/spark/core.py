...
class _SparkXGBSharedReadWrite:
    ...
    def saveMetadata(
        ...
        # DefaultParamsWriter.saveMetadata might require sc, but in Connect we might need to use
        # a different approach or assume it works with session if updated.
        # However, standard PySpark DefaultParamsWriter uses sc.
        # If we are in Connect, we might need to use pyspark.ml.util.DefaultParamsWriter from Connect?
        # Assuming standard one works or we pass None for sc if it allows.
        # Actually, saveMetadata uses sc to write to path.
        # In Connect, we should use session.
        # But DefaultParamsWriter.saveMetadata signature is fixed.
        # We might need to implement our own saveMetadata if DefaultParamsWriter is not compatible.
        # For now, we pass None as sc and hope it uses internal session or fails gracefully?
        # No, it uses sc.parallelize(...).saveAsTextFile(...)
        # We should reimplement saveMetadata logic using session.
